...
// Proxy base (utilisé pour les URLs de flux)
const PROXY_BASE = (process.env.PROXY_BASE || "https://vavoo-iptv-proxy.vavoo-iptv.workers.dev").replace(/\/+$/, "");
// EPG upstream (par défaut epgshare01 pour la Turquie, mais tu peux en changer)
...
function normalizeForCategory(name) {
  let s = String(name || "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  // Restauration des caractères turcs (peut être utile)
  s = s
...
const CATEGORY_RULES = [
  ...
  {
    name: "Çocuk",
    re: /CARTOON|BOOMERANG|DISNEY|NICK(?:ELODEON|TOONS|JR|JUNIOR|\b)|BABY ?TV|BABYTV|M[İI]?N ?KA|MINIKA|POKEMON|POKÉMON|ANIMATION|ANIMASYON|TRT ?[ÇC]?OCUK|OCUK HD|\bCOCUK\b|\b[ÇC]OCUK\b|BEN ?10|ANGRY BIRDS|CAILLOU|PEPPA|PEPE|HEIDI|SIRINLER|TOM & JERRY|S[ÜU]NGER|SPIDERMAN|BARBIE|PIJAMA|PIRIL|RAFADAN|KELOGLAN|KUKULI|KOSTEBEK|CHICKY|BOOBA|WAKFU|GABBY|TAYO|NILOYA|PISI|LEYLEK|MASAL|CANIM KARDESIM|ADIBESA|MOMO|ALVIN|VIKINGLER|TRANSFORMERS|TROL AVCILARI|SMART COCUK|ILAHI COCUK|CILGIN ORMAN|KRAL SAKIR|SERCE KUS|ITFAYECI SAM|MUFFETIS|MAYMUNLAR|ELIF VE|ELIFIN|MIMOCAN|HAPSUU|RUYA TRENI|MASA KOCAAYI|PAK PIRPIR|LIMON ZEYTIN|GONCA TV|NASREDDIN|SEKER HOCA|SEVIMLI DOSTLAR|PAW PETROL|OSCAR COLLERDE|SL NILOYA|CBEEBIES|DUCK TV|JIM ?JAM|ENGLISH CLUB TV|EBA TV|TAV[SŞ]AN|PATRON BEBEK|D[İI]YARI|BAHA\b|SEF ROKKA|BULMACA KULESI|AKILLI TAV[SŞ]AN|AKLILI|CANIM KARDESIM|DA VINC KIDS|DA VINCI KIDS|DINAMIK ANIMASYON|DREAM ANIMASYON|MAX ANIMASYON|ENO ANIMASYON|BEST ANIMASYON|YILDIZ KIZ|KONU[SŞ]AN TOM|JURASSIC WORLD|MONTAG/i,
  },
  {
    name: "Belgesel",
...
async function main() {
  ...
  if (COUNTRY_FILTER) {
    console.log(`Filtering by country: ${COUNTRY_FILTER}`);
  } else {
    console.log("No country filter – fetching all channels.");
  }
  if (PROXY_BASE) {
    console.log(`Using PROXY_BASE=${PROXY_BASE}`);
  } else {
    console.warn(
      "WARNING: PROXY_BASE is empty. Raw vavoo.to URLs will be written."
    );
  }
  ...
  // Tri pour différences stables
  items.sort((a, b) => {
    const an = String(a.name ?? "").toLocaleLowerCase("fr-FR");
    const bn = String(b.name ?? "").toLocaleLowerCase("fr-FR");
    if (an < bn) return -1;
    if (an > bn) return 1;
    const ai = a.ids?.id ?? "";
    const bi = b.ids?.id ?? "";
  ...
  try {
    ...
    console.warn(
      `Upstream EPG unavailable (${err.message}); using Vavoo inline EPG only.`
    );
  }
  ...
  console.log(`EPG matched ${matched}/${items.length} channels`);
